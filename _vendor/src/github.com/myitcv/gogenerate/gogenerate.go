...
// FileIsGenerated determines wheter the file located at path is generated or not
func FileIsGenerated(path string) bool {
	return strings.HasPrefix(filepath.Base(path), GenFilePrefix)
}

// FileGeneratedBy returns true if the base name of the supplied path is a file that
...
func FileGeneratedBy(path string, cmd string) bool {
	...
	return strings.HasSuffix(fn, augCmd(cmd)+".go") || strings.HasSuffix(fn, augCmd(cmd)+"_test.go")
}

func augCmd(cmd string) string {
	return "_" + cmd
}
...
func NameFile(name string, cmd string) string {
	...
	return fmt.Sprintf("%v%v%v.go", GenFilePrefix, name, augCmd(cmd))
}
...
func NameTestFile(name string, cmd string) string {
	...
	return fmt.Sprintf("%v%v%v_test.go", GenFilePrefix, name, augCmd(cmd))
}
